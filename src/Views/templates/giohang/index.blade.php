                        ...
                        <div class="top-cart">
                            ...
                            <div class="list-procart">
                                ...
                                @foreach (Cart::content() as $k => $v)
                                    ...
                                    <div
                                        class="procart flex align-items-center justify-between procart-{{ $v->rowId }}">
                                        <div class="pic-procart">
                                            <a class="text-decoration-none"
                                                href="{{ url('slugweb', ['slug' => $proInfo->slugvi]) }}" target="_blank"
                                                title="{{ $proInfo->namevi }}">
                                                <img src="{{ assets_photo('product', '100x100x1', $proInfo->photo, 'thumbs') }}"
                                                    alt="{{ $proInfo->namevi }}" />
                                            </a>
                                        ...
                                        <div class="info-procart">
                                            <h3 class="name-procart"><a class="text-decoration-none"
                                                    href="{{ url('slugweb', ['slug' => $proInfo->slugvi]) }}"
                                                    target="_blank"
                                    ...
                                    @foreach ($httt as $k => $v)
                                        <div class="payments-cart form-check">
                                            <input type="radio" class="form-check-input"
                                                id="payments-{{ $v->id }}" name="dataOrder[payments]"
                                                value="{{ $v->id }}" required>
                                            <label class="payments-label form-check-label"
                                                for="payments-{{ $v->id }}"
                                                data-payments="{{ $v->id }}">{{ $v->namevi }}</label>
                                            <div class="payments-info payments-info-{{ $v->id }} transition">
                                                {!! Func::decodeHtmlChars($v->descvi) !!}</div>
                                        </div>
                                    @endforeach
                                </div>
