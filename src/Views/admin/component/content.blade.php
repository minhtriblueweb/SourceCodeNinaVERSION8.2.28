    ...
    <div class="tab-content" id="custom-tabs-three-tabContent-lang">
        @foreach (config('app.langs') as $k => $v)
            <div class="tab-pane fade show {{ $k == 'vi' ? 'active' : '' }}" id="tabs-lang-{{ $k }}"
                ...
                @if (!empty($quantity))
                    <div class="form-group last:!mb-0">
                        <label class="form-label" for="quantity">Số liệu :</label>
                        <input type="number" min="0" class="form-control for-seo text-sm" name="data[quantity]"
                            id="quantity" placeholder="Số liệu"
                            value="{{ !empty(Flash::has('quantity')) ? Flash::get('quantity') : $item['quantity'] }}"
                            required>
                    </div>
                @endif
                ...
                @if (!empty($address))
                    <div class="form-group last:!mb-0">
                        ...
                        <input type="text" class="form-control for-seo text-sm" name="data[address]" id="address"
                            placeholder="Địa chỉ"
                ...
                @if (!empty($phone))
                    <div class="form-group last:!mb-0">
                        ...
                        <input type="text" class="form-control for-seo text-sm" name="data[phone]" id="phone"
                            placeholder="Điện thoại"
                            value="{{ !empty(Flash::has('phone')) ? Flash::get('phone') : $item['phone'] }}" required>
                    </div>
                @endif

                @if (!empty($iframe))
                    <div class="form-group last:!mb-0">
                        <label class="form-label" for="iframe">Iframe maps
                            :</label>
                        <textarea class="form-control for-seo text-sm {{ !empty($iframe_cke) ? 'form-control-ckeditor' : '' }}"
                            name="data[iframe]" id="iframe" rows="5" placeholder="Iframe maps">{{ !empty(Flash::has('iframe')) ? Flash::get('iframe') : @$item['iframe'] }}</textarea>
                    </div>
                @endif
                ...
                @if (!empty($promotion))
                    <div class="form-group last:!mb-0">
                        <label class="form-label" for="promotion{{ $k }}">Khuyến mãi ({{ $k }})
                            :</label>
                ...
                @if (!empty($incentives))
                    <div class="form-group last:!mb-0">
                        <label class="form-label" for="incentives{{ $k }}">Tài liệu ({{ $k }})
                            :</label>
                        <textarea class="form-control for-seo text-sm {{ !empty($incentives_cke) ? 'form-control-ckeditor' : '' }}"
                            ...
                            placeholder="Tài liệu ({{ $k }})">{{ !empty(Flash::has('incentives' . $k)) ? Flash::get('incentives' . $k) : @$item['incentives' . $k] }}</textarea>
                    </div>
                @endif
                @if (!empty($desc))
                    <div class="form-group last:!mb-0">
